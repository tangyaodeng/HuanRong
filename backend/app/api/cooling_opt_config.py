#backend/app/api/cooling_opt_config.py
...
import logging
from ..crud.cooling_opt_config import CoolingOptCRUD
import redis
import json
...
router = APIRouter(prefix="/cooling_opt_config", tags=["冷却侧优化"])

# 初始化CRUD
cooling_opt_crud = CoolingOptCRUD()
...
@router.get("/iteration/latest", response_model=schemas.CoolingOptIterationResponse)
async def get_latest_iteration():
    ...
    try:
        ...
        try:
            ...
            # 直接返回解析后的字典，Pydantic 会自动验证
            return parsed
        except json.JSONDecodeError as e:
            logger.error(f"Redis 数据 JSON 解析失败: {e}, data={data}")
            raise HTTPException(status_code=500, detail="迭代数据格式错误")
    except redis.RedisError as e:
        logger.error(f"Redis 操作失败: {e}")
        raise HTTPException(status_code=503, detail="Redis 服务异常")
    except Exception as e:
        logger.error(f"获取迭代数据失败: {e}")
        raise HTTPException(status_code=500, detail="获取迭代数据失败")

@router.get("/iteration/page", response_model=schemas.CoolingOptIterationPageResponse)
...
@router.get("/config", response_model=schemas.CoolingOptConfigResponse)
async def get_config(db: Session = Depends(get_db)):
    """获取冷却侧优化配置"""
    try:
        config = cooling_opt_crud.get_config(db)
        if not config:
            config = models.CoolingOptConfig()
            db.add(config)
            db.commit()
            db.refresh(config)
            logger.info("创建了默认冷却侧优化配置")
        return config
    except Exception as e:
        logger.error(f"获取配置失败: {str(e)}")
        raise HTTPException(status_code=500, detail="获取配置失败")

@router.put("/config", response_model=schemas.CoolingOptConfigResponse)
async def update_config(
        config_update: schemas.CoolingOptConfigUpdate,
        db: Session = Depends(get_db)
):
    """更新冷却侧优化配置"""
    try:
        config = cooling_opt_crud.update_config(db, config_update)
        logger.info("冷却侧优化配置已更新")
        return config
...
@router.get("/parameters", response_model=schemas.CoolingOptParametersTotalList)
async def get_parameters(
    ...
    """获取冷却侧优化参数列表"""
    try:
        parameters = cooling_opt_crud.get_parameters(
            db, skip=skip, limit=limit, applied=applied,
            start_date=start_date, end_date=end_date
        )
        total = cooling_opt_crud.count_parameters(
            db, applied=applied, start_date=start_date, end_date=end_date
        )
        total_pages = (total + limit - 1) // limit if limit > 0 else 1
        current_page = (skip // limit) + 1 if limit > 0 else 1
        return schemas.CoolingOptParametersTotalList(
...
@router.get("/parameters/latest", response_model=schemas.CoolingOptParametersTotalResponse)
async def get_latest_parameters(db: Session = Depends(get_db)):
    """获取最新的冷却侧优化参数"""
    try:
        parameters = cooling_opt_crud.get_latest_parameters(db)
        if not parameters:
...
@router.get("/parameters/{parameter_id}", response_model=schemas.CoolingOptParametersTotalResponse)
async def get_parameter_by_id(
    ...
    """根据ID获取冷却侧优化参数"""
    try:
        parameter = cooling_opt_crud.get_parameter_by_id(db, parameter_id)
        if not parameter:
            raise HTTPException(status_code=404, detail="未找到优化参数")
        return parameter
    except HTTPException:
...
async def get_optimization_stats(
    ...
    """获取优化统计信息"""
    try:
        stats = cooling_opt_crud.get_optimization_stats(db, days)
        return stats
    except Exception as e:
        logger.error(f"获取统计信息失败: {str(e)}")
        raise HTTPException(status_code=500, detail="获取统计信息失败")

@router.get("/parameters/history", response_model=List[schemas.CoolingOptParametersTotalResponse])
...
async def get_device_field_history(
    ...
    """获取指定设备字段的历史数据（当前值和优化值）"""
    try:
        data = cooling_opt_crud.get_device_field_history(db, field, start_date, end_date, limit)
        return data
