...
class TrainingScheduler:
    ...
    def recover_expired_schedules(self):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"✅ 已成功恢复 {recovered_count} 个任务计划")
    ...
    def get_pending_schedules_by_type(self, schedule_type: str, time_window_minutes: int = 5):
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"检查{schedule_type}任务，当前时间（东八区）: {now}，窗口结束: {window_end}")
    ...
    def execute_schedule_task_directly(self, schedule_id: int):
        ...
        try:
            ...
            try:
                ...
                # 计算下次运行时间
                if schedule_record.end_time is None or schedule_record.last_run_at < schedule_record.end_time:
                    if schedule_record.interval_unit == 'minutes':
                        delta = timedelta(minutes=schedule_record.interval_value)
                    elif schedule_record.interval_unit == 'hours':
                        delta = timedelta(hours=schedule_record.interval_value)
                    else:  # days
                        delta = timedelta(days=schedule_record.interval_value)

                    schedule_record.next_run_at = schedule_record.last_run_at + delta
                    logger.info(f"计划 {schedule_id} 下次执行时间: {schedule_record.next_run_at}")
                else:
    ...
    def execute_real_prediction(self, device_id: int):
        ...
        try:
            ...
            # 执行预测
            logger.info(f"🔮 开始执行预测，设备: {device_id}")
                ...
                if pending_schedules:
                    ...
                    for schedule in pending_schedules:
                        try:
                            ...
                            if schedule.device_id in [task.get('device_id') for task in
                                                      queue_status.get('active_tasks', [])]:
    ...
    def execute_predict_schedules(self):
        """执行预测任务 - 从整点开始每5分钟检查一次"""
        with self.predict_lock:
            try:
                logger.debug("检查待执行预测计划...")
                self.task_statistics['last_predict_check'] = datetime.now()

                # 获取接下来5分钟内的预测任务（因为预测最少5分钟一次）
                pending_schedules = self.get_pending_schedules_by_type('predict', time_window_minutes=5)
    ...
    def _calculate_next_run(self, schedule):
        ...
        # 检查是否超过结束时间
        if schedule.end_time and schedule.next_run_at > schedule.end_time:
            schedule.is_active = False
            logger.info(f"计划 {schedule.id} 已到达结束时间，自动停止")
        else:
            logger.info(f"计划 {schedule.id} 下次执行时间: {schedule.next_run_at}")
    ...
    def monitor_task_queue(self):
        ...
        # 检查是否有任务卡住
        for task_id in queue_status['active_task_ids']:
    ...
    def _train_thread_worker(self):
        """训练任务线程工作函数 - 修正：每小时检查一次"""
        logger.info("🚀 训练任务线程启动")

        # 等待到下一个整点
        current_time = datetime.now()
        seconds_to_wait = (3600 - current_time.minute * 60 - current_time.second)
        if seconds_to_wait > 0:
            logger.info(f"训练线程等待 {seconds_to_wait} 秒到下一个整点")
    ...
    def _predict_thread_worker(self):
        ...
        if seconds_to_wait > 0:
            logger.info(f"预测线程等待 {seconds_to_wait} 秒到下一个整5分钟")
            time.sleep(seconds_to_wait)
